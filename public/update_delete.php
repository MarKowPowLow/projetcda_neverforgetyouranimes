...
$anime_id = isset($_GET['anime_id']) ? htmlspecialchars($_GET['anime_id']) : (isset($_POST['select_anime_id']) ? htmlspecialchars($_POST['select_anime_id']) : $animeData['anime_id']);

// Appel de la fonction searchAnimeInAnimesList puis stockage du retour dans une variable afin d'avoir accès à l'anime qu'on veut éditer.
$animeData = searchAnimeInAnimesList($pdo, (int) $get_user_id, (int) $anime_id);

// Stockage de l'id de l'animeList dans une variable.
$get_animeList_id = isset($_POST['hidden_animeList_id']) ? htmlspecialchars($_POST['hidden_animeList_id']) : $animeData['id'];

// Vérification de l'id du langage dans la superglobale $_POST puis stockage de cet ID de manière sécurisée avec htmlspecialchars. Si l'id n'est pas encore défini, on lui donne une valeur par défaut obtenue dans animeData.
$language_id = isset($_POST['select_language_id']) ? htmlspecialchars($_POST['select_language_id']) : $animeData['language_id'];

// Appel la fonction searchLanguageForAnime qui va venir vérifier quel est ou quels sont les langages disponible pour l'anime en question.
...
// Définition de status_id
$status_id = isset($_POST['select_status_id']) ? htmlspecialchars($_POST['select_status_id']) : $animeData['status_id'];

// Appel la fonction searchEpisodesForAnime qui va venir récupérer les épisodes disponible pour l'anime et le langage en question.
$episodesData = searchEpisodesForAnime($pdo, $anime_id, $language_id);

// Définition de episode_number
$episode_number = isset($_POST['select_episode_id']) ? htmlspecialchars($_POST['select_episode_id']) : $animeData['current_episode'];

// Appel la fonction searchLastEpisode qui va venir récupérer le dernier épisode sorti pour l'anime et le langage en question.
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Vérification de l'existance de notre variable action ainsi que du nom de l'action (Modifier l'anime).
    if (isset($_POST['action']) && $_POST['action'] === "Modifier l'anime") {
        // Vérification de l'existance des variables dans notre superglobale $_POST ou dans notre variable $animeData, mais également de leur non nullité.
        if (
            ...
            ||
            isset($animeData['anime_id'], $animeData['status_id'], $animeData['language_id'], $animeData['current_episode'], $animeData['last_released_episode'], $get_user_id) &&
            !empty($animeData['anime_id']) && !empty($animeData['status_id']) && !empty($animeData['language_id']) && !empty($animeData['current_episode']) && !empty($animeData['last_released_episode']) && !empty($get_user_id)
        ) {
            ...
            $animeList_validate_id = cleanAndValidateValue($animeData['id']);
            $user_validate_id = cleanAndValidateValue($_POST['hidden_user_id']);
            $anime_validate_id = cleanAndValidateValue($_POST['select_anime_id']);
            $status_validate_id = cleanAndValidateValue($_POST['select_status_id']);
            $language_validate_id = cleanAndValidateValue($_POST['select_language_id']);
            $current_episode_validate = cleanAndValidateValue($_POST['select_episode_id']);
            $last_released_episode_validate = cleanAndValidateValue($_POST['hidden_last_released_episode']);
            ...
            $success = updateAnimesList($pdo, $current_episode_validate, $last_released_episode_validate, $user_validate_id, $anime_validate_id, $status_validate_id, $language_validate_id, $animeList_validate_id);
            ...
            if ($success) {
                unset($_POST['action']);
                $messageSucceeded = "L'anime a bien été modifier !";
            } else {
                // Sinon, on envoie un message signalant une erreur lors de l'opération.
                $messageFailed = "Une erreur s'est produite lors de la modification de votre anime à la liste.";
            }
        } else {
    ...
    // Vérification de l'existance de notre variable action ainsi que du nom de l'action (Supprimer l'anime).
    if (isset($_POST['action']) && $_POST['action'] === "Supprimer l'anime") {
        if ( // Vérification de l'existance de la variable dans notre superglobale $_POST ou dans notre variable $animeData, mais également de sa non nullité.
            isset($_POST['hidden_animeList_id']) && !empty($_POST['hidden_animeList_id'])
            ||
            isset($animeDat['id']) && !empty($animeData['id'])
        ) {
            $animeList_validate_id = cleanAndValidateValue($animeData['id']);

            $deleteSuccess = deleteAnimesInList($pdo, $animeList_validate_id);

            // Vérification de la réussite de l'opération (C'est un boolean), si l'opération à réussi, on nettoie notre superglobale $_POST de toutes les données et on affiche un message de réussite (dans la suppression de l'anime).
            if ($deleteSuccess) {
                ...
                unset($_POST['action']);
                $messageSucceeded = "L'anime a bien été supprimer !";
            } else {
                // Sinon, on envoie un message signalant une erreur lors de l'opération.
                $messageFailed = "Une erreur s'est produite lors de la suppression de votre anime à la liste.";
            }
        }
    }
}
...
<!-- Affichage du message d'échec du formulaire -->
    ...
    <div class="container mx-auto py-8">
        <form action="update_delete.php" method="post" class="w-full max-w-sm mx-auto bg-white p-8 rounded-md shadow-md">
            <label for="hidden_user_id">
            <input type="text" id="hidden_user_id" name="hidden_user_id" value="<?=$get_user_id?>" hidden>
            <label for="hidden_animeList_id">
            <input type="text" id="hidden_animeList_id" name="hidden_animeList_id" value="<?=$get_animeList_id?>" hidden>
            <div class="mb-4">
                ...
                <!-- Affichage du nom de l'anime à éditer. -->
                    <option value="<?=$animeData['anime_id']?>"><?=$animeData['anime_name']?></option>
                </select><br><br>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="select_status_id" id="select_status_id" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                    ...
                    <option value="<?=$statusData['id']?>"<?=(isset($status_id) && $statusData['id'] == $status_id) ? 'selected' : ''?>>
                        <?=$statusData['name']?>
                    </option>
                    <?php endforeach;?>
                </select><br><br>
            ...
            <div class="mb-4">
                ...
                <select name="select_episode_id" id="select_episode_id" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                    ...
                    <option value="<?=$episodeData['episode_number']?>"<?=(isset($episode_number) && $episodeData['episode_number'] == $episode_number) ? 'selected' : ''?>>
            ...
            <div class="flex justify-around items-center mb-4">
                <div>
                    <label for="deleteAnime">
                    <button
                    class="w-32 bg-red-500 text-white text-sm font-bold py-2 px-4 rounded-md hover:bg-red-700 transition duration-300"
                    type="submit" name="action" id="deleteAnime" value="Supprimer l'anime">Supprimer l'anime
                    </button>
                </div>
                ...
                <div>
                    <label for="editAnime">
                    <button
                    class="w-32 bg-indigo-500 text-white text-sm font-bold py-2 px-4 rounded-md hover:bg-indigo-600 transition duration-300"
                    type="submit" name="action" id="editAnime" value="Modifier l'anime">Éditer l'anime
                    </button>
                </div>
            </div>
        </form>
